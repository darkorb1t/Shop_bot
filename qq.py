...
# --- CONFIGURATION ---
TOKEN = 'YOUR_BOT_TOKEN_HERE'       # তোমার টেলিগ্রাম টোকেন
ADMIN_ID = 123456789                # তোমার অ্যাডমিন আইডি
...
# --- FAKE SERVER (For 24/7 Uptime/Monitoring) ---
app = Flask(__name__)
...
async def role_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'role_cust':
        ...
        await q.message.reply_text(TEXTS[lang]['res_ask_id'], parse_mode='Markdown')
...
async def reseller_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'awaiting_pass' in context.user_data:
        ...
        if c.fetchone():
            ...
            del context.user_data['awaiting_pass']
            await update.message.reply_text(TEXTS[lang]['res_fail'])
            conn.close()
            return await ask_role_screen(update, context, lang)
    ...
    if c.fetchone():
        ...
        await update.message.reply_text(TEXTS[lang]['res_ask_pass'], parse_mode='Markdown')
        conn.close()
        return RESELLER_INPUT
    else:
        await update.message.reply_text(TEXTS[lang]['res_fail'])
...
async def universal_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == 'menu_0': # Shop
        ...
        if not prods:
            ...
            conn.close()
            return MAIN_STATE
        await q.message.reply_text("🛒 **SHOP ITEMS:**")
        for p in prods:
        ...
        conn.close()
        return MAIN_STATE
    elif d == 'menu_1': await q.message.reply_text(t['profile'].format(user[1], uid, user[4], user[3]), parse_mode='Markdown')
    elif d == 'menu_2': 
        ...
        return INPUT_COUPON
    elif d == 'menu_4': await q.message.reply_text(f"🤝 Refer Link:\n`https://t.me/{context.bot.username}?start=ref_{uid}`\nBonus: 1 Tk", parse_mode='Markdown')
    elif d == 'menu_5': await q.message.reply_text(t['support'].format(ADMIN_USERNAME))
...
async def input_money(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await
      update.message.reply_text(TEXTS[u[2]]['ask_trx'].format(amt, amt, BKASH_NUMBER), parse_mode='Markdown')
        return INPUT_TRX
    except: 
...
async def input_coupon(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if res and res[3] < res[2]:
        ...
        conn.commit()
        await update.message.reply_text(TEXTS[get_user(update.effective_user.id)[2]]['coupon_applied'].format(res[1]))
...
def main():
    # Initialize DB (Check connection once)
    init_db()
    
    # Start Keep Alive for 24/7 (Optional for EC2 but requested)
    keep_alive()
    ...
    print("Bot Started 24/7 with NeonDB...")
    app.run_polling()
